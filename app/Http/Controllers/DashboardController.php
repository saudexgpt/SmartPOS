...
class DashboardController extends Controller
{
    ...
    {
        $startDate = $this->dateService->copy()->startOfMonth();
        $endDate = $this->dateService->copy()->endOfMonth();
        if(isset($request->start_date, $request->end_date) && $request->start_date != '' && $request->end_date != '') {
            $startDate = date('Y-m-d H:i:s', strtotime($request->start_date));
            $endDate = date('Y-m-d H:i:s', strtotime($request->end_date));
        }
        $orders = OrderProduct::groupBy('product_id')
        ->select('*', \DB::raw('SUM(total_price) as amount'))
